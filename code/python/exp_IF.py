...
def getting_started():
    ...
    state_monitor, spike_monitor = simulate_exponential_IF_neuron(
        I_stim=input_current, simulation_time=150 * b2.ms)
    plot_tools.plot_voltage_and_current_traces(
        state_monitor, input_current, title="Simulation of exponential neuron model", firing_threshold=FIRING_THRESHOLD_v_spike)
    ...
    # (state_monitor, spike_monitor) = simulate_exponential_IF_neuron(
    #     I_stim=sinusoidal_current, simulation_time=200 * b2.ms)

    # plot_tools.plot_voltage_and_current_traces(
    #     state_monitor, sinusoidal_current, title="Sinusoidal input current", firing_threshold=FIRING_THRESHOLD_v_spike)
    # print("nr of spikes: {}".format(spike_monitor.count[0]))
    # plt.show()
